...
class ViewWorkout : AppCompatActivity() {
    ...
    private fun fetchListOfExercises(clickedWorkoutTitle: String) {
        ...
        uid?.let {
            db.collection("userExercise").document(uid)
                .collection("listOfWorkouts").document(clickedWorkoutTitle) // Use the clicked workout title
                .collection("listOfExercises")
                .get()
                .addOnSuccessListener { documents ->
            ...
            db.collection("userExercise").document(uid)
                ...
                .addOnSuccessListener { documents ->
                    ...
                    for (document in documents) {
                        ...
                        // Logging each retrieved document
                        Log.d(TAG, "Document ID: ${document.id}, Exercise Title: $exerciseTitle")
                    }
                    // Pass the list of exercise titles to updateRecyclerView function
                    updateRecyclerView(exerciseTitlesList)
                }
                .addOnFailureListener { exception ->
                    Log.e(TAG, "Error getting documents: ", exception)
